     ...
     - *In Java*
       ...
       - Defining and starting a Thread
         #+BEGIN_SRC lang-java
          ...
          #+END_SRC
       - The =join()= method will block until the thread finishes.
...
** DONE Code Smells and Refactoring
   - *Refactoring*
     ...
     - *Refactoring Methods*
       - *Extract Method*
         - turn fragment of code into a method whose name explains the
           purpose of the method
       - *Extract Class*
         - you have one class doing work that should be done by two.
         - create a new class and move the relevant fields and methods
           from the old class into the new clas.
       - *Replace Data Value with Object*
         - you have a data item that needs additional data or
           behavior.
         - turn the data item into an object.
       - *Hide Delegate*
         - A client is calling a delegate class of an object.
         - create methods to hide the delegate
       - *Replace Conditional w/ Polymorphism*
         - you have a conditional that chooses different behavior
           depending on the type of an object
         - Move each leg of the conditional to an overriding method in
           a subclass. Make the original method abstract.
   - *Code Smells*
     - *Duplicated Code*
...
** DONE Black-Box Testing
   ...
   - *Equivalence Class Testing*
     ...
     - *Weak Robust*
       - considers invalid values, single fault assumption
     - *Strong Robust*
...
** DONE Debugging
   - Murphy's levels
     1. *Deduction*: read the code and try to reason about it
     2. *Observation*: execute the code once (maybe using a logger or
        debugger) and consider the values of variables, perhaps in
        relation to expected invariants, and look for transition from
        good state to error state
     3. *Induction*: compare properties (black-box coverage, white-box
        coverage) of passing and failing test cases and look for
        differences that may be indicative of the cause
     4. *Experimentation*: modify the code so that failing tests pass,
        then compare differences between failing and passing tests
   - "Scientific Method" (from reading)
     - *Stabilize the error*
